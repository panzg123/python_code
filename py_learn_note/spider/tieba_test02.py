...
class HTML_Tool:
    ...
    def Replace_Char(self,x):
        x = self.BgnCharToNoneRex.sub("",x)  #处理图片，链接等
        x = self.BgnPartRex.sub("\n    ",x)  #处理<p>标签
        x = self.CharToNewLineRex.sub("\n",x)#处理<dvi>等标签
        x = self.CharToNextTabRex.sub("\t",x)#处理<td>标签
        x = self.EndCharToNoneRex.sub("",x)

        for t in self.replaceTab:  
            x = x.replace(t[0],t[1])    # 将一些html的符号实体转变为原始符号
        return x  
...
class Baidu_Spider:
    ...
    def find_title(self,myPage):
        # 匹配 <h3 class="core_title_txt" title="xxxx">新闻标题</h3> 找出标题
